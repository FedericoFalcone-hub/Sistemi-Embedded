...
\section{Lezione 13}
Si può testare il comportamente di un condensatore collegandolo ad un GPIO analogico.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images/condensatore_board.png}
\end{figure}
Scrivendo high carico il condensatore, con low lo scarico. Sul GPIO analogico posso usarlo come tester, ovvero riesco a capire a che punto è arrivato il condensatore.
\\ Vedi codice \href{run:../codice/PlotCapacitor/PlotCapacitor.ino}{PlotCapacitor.ino}.
\\ Per avere un'etichetta nel plot, basta che nella print si fa Serial.print("Etichetta:") e poi Serial.print(valore). Nel codice, C è il valore del condensatore (Farad) calcolato.
\subsection{Transistor}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.25\textwidth]{Images/transistor_npn.png}
\end{figure}
Il transistor è (ai nostri fini) quasi un interruttore. È nato come amplificatore, pilotare qualcosa di grande con qualcosa di piccolo. Meccanismo che permette di leggere il carico per pilotare per esempio un motore, ma usando una roba molto piccola (come GPIO) come consenso, meccanismo di attivazione o disattivazione.
\\ B$\rightarrow$ base, E$\rightarrow$ emettitore, C$\rightarrow$ collettore. Solitamente emittore ground, collettore alimantazione. Tra emittotore e collettore scorre una corrente che è funzione della corrente che passa tra base ed emittitore. C-E fa passare la corrente solo se passa della corrente  (poca) tra B-E. Il flusso di corrente tra B-E permette di regolare il flusso tra C-E
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images/transistor_circuito.png}
\end{figure}
Quanta corrente serve tra base ed emettitore per far passare una certa corrente? Molto poca. Con troppa corrente si brucia. Sotto gli 0.6-0.7V si spegne. Abbassando la base, si abbassa anche la corrente che passa tra C-E.
\subsection{Ponte H}
Utilizzato di solito per pilotare comodamente la velocità di rotazione di un motore.*
\section{Lezione 14}
\subsection{Comunicazione "locale"}
Comunicazione locale elettrica, nelle vicinanze.
\\ Modulazione. Comunicazione con PWM, in cui si modula il duty cycle per trasmettere informazioni.
\\ Vedi documento \href{run:https://gitlab.di.unimi.it/sistemiembedded/2024-2025/-/blob/master/comunicazioneLocale.md?ref_type=heads}{ComunicazioneLocale}.
\\ In caso di segnale in ingresso digitale, il mittente prima trasmette e poi alza il clock, così non si rischia di leggere nel momento di indeterminatezza di quando il valore sta salendo o scendendo. Se il clock è \textbf{anticipato} rispetto al segnale prende sempre gli 0, se invece è lievemente \textbf{posticipato} sono certo di prendere almeno il primo \texttt{high}.
Il periodo del clock deve essere confrontabile con quello del segnale: è il clock stesso che dice quanto dura un bit.
\\ Cosa mi serve per una comunicazione?
\begin{itemize}
    \item GND (riferimento comune per la tensione);
    \item dati (half o full duplex);
    \item clock (e se vogl);
    \item select o bus (informazione che mi dice a chi voglio parlare. Utile se condivido i primi 3).
\end{itemize}
Per avere un clock interno che si sincronizza con il segnale, azzero il clock dopo pochi ms dall'inizio del segnale. Una volta letti i bit decisi dal protocollo, azzero tutto.
\section{Lezione 15}
\subsection{Bit banging}
In caso non avessi dell'hardware per il clock, posso utilizzare il \textbf{bit banging}.
\subsection{Comunicazione "lontana"}
Comunicazione fatta wi-fi/bluethooth. Uno dei protocolli più utilizzati è MQTT (TCP/IP). È basato su un pattern chiamato Publish-Subscribe, il quale si basa che i fornitori di informazioni (pubblicatori) e i consumatori (sottoscrittori) non si conoscono, cioè un publisher non sa quanti subscriber ci sono per le info che sta generando. Si trovano perchè il mittente invia l'informazione etichettata, il ricevente dichiara di essere interessato all'informazione etichettata.
Il meccanismo è semplice, si basa sul fatto che esistono, oltre ai due ruoli, esistono i \textbf{broker}, che gestiscono le informazioni, che sono i "centralini" che raccolgono le info che vengono pubblicate dai publisher e le distribuiscono ai subscriber.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images/broker.png}
\end{figure}
Il singolo nodo che si sveglia deve sapere qual è il broker al quale connettersi, se vuol fare la pubblisher si connette al broker e quando viene pubbliato dice al broker ho un messaggio pronto con questa etichetta. Il subscriber deve connettersi al broker e deve elencare al broker quali sono i suoi interessi (quali etichette), da quel momento rimane in contatto con il socket aperto verso il broker e quando il broker incontra, gli vengono dati messaggi  con quelle etichette che ha chiesto, gli gira il messaggio.
A quel punto farà una certa cosa.
\\ Il messaggio MQTT è formato da 2 componenti principali: il \textbf{topic}, etichetta e un \textbf{payload} che è il contenuto del messaggio. Esempio: Appartamento/Sala/Temperatura $<$valore$>$.Il topic è una stringa. Se io fossi interessato a tutte le temperature di tutto l'appartamento, c'è un meccanismo di wildcard. Quello che viene dopo è il corpo.
\\ Dico al broker che sono interessato ad un topic, quando il broker ci inoltrerà il messaggio viene invocata una funzione \textbf{callback} che ha come parametro tra i suoi il payload
\\ Wildcards: \# prende tutti i topic e tutti i subtopic. + tutti i topic ma senza i subtopic.
\\ Mosquitto.
